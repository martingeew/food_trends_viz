...
fig.patch.set_facecolor(
    GREY98
)  # sets colour of entire background (incl. outside the plot area)
...
xmin = data.index.min()
xmax = data.index.max() + pd.DateOffset(
    years=2
)  # add space to the right of the lines for labels
...
ax.set_xticklabels(
    x_tick_labels.strftime("%Y"), weight=500, color=GREY40, fontfamily=font_family
)
...
ax.set_yticklabels(y_ticks, weight=500, color=GREY40, fontfamily=font_family)
